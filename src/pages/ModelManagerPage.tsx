        ...
        {LOCAL_MODELS.map((model) => {
          ...
          return (
            <button type="button" key={model.id} className="text-left" onClick={() => setCurrentModelId(model.id)}>
              <GlassCard
                className={`flex h-full flex-col gap-4 p-4 transition ${
                  active ? 'border-cyan-400/30 bg-cyan-400/10' : 'hover:border-white/20 hover:bg-white/10'
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full border border-white/10 px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-slate-500">
                    {model.status}
                  </span>
                </div>
